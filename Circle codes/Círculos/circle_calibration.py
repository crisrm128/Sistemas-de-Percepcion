...
N = 72 #Distancia entre círculos (para demostrar que este parámetro no influye en el error, simplemente se necesita la relación)
...
objp[4]  = (N/2 , N/2 , 0)
objp[5]  = (N/2 , N+N/2, 0)
objp[6]  = (N/2 , 2*N+N/2, 0)
objp[7]  = (N/2 , 3*N+N/2, 0)
objp[8]  = (N , 0  , 0)
objp[9]  = (N , N , 0)
objp[10] = (N , 2*N, 0)
objp[11] = (N , 3*N, 0)
objp[12] = (N+N/2, N/2,  0)
objp[13] = (N+N/2, N+N/2, 0)
objp[14] = (N+N/2, 2*N+N/2, 0)
objp[15] = (N+N/2, 3*N+N/2, 0)
objp[16] = (2*N, 0  , 0)
objp[17] = (2*N, N , 0)
objp[18] = (2*N, 2*N, 0)
objp[19] = (2*N, 3*N, 0)
objp[20] = (2*N+N/2, N/2 , 0)
objp[21] = (2*N+N/2, N+N/2, 0)
objp[22] = (2*N+N/2, 2*N+N/2, 0)
objp[23] = (2*N+N/2, 3*N+N/2, 0)
objp[24] = (3*N, 0  , 0)
objp[25] = (3*N, N , 0)
objp[26] = (3*N, 2*N, 0)
objp[27] = (3*N, 3*N, 0)
objp[28] = (3*N+N/2, N/2 , 0)
objp[29] = (3*N+N/2, N+N/2, 0)
objp[30] = (3*N+N/2, 2*N+N/2, 0)
objp[31] = (3*N+N/2, 3*N+N/2, 0)
objp[32] = (4*N, 0  , 0)
objp[33] = (4*N, N , 0)
objp[34] = (4*N, 2*N, 0)
objp[35] = (4*N, 3*N, 0)
objp[36] = (4*N+N/2, N/2 , 0)
objp[37] = (4*N+N/2, N+N/2, 0)
objp[38] = (4*N+N/2, 2*N+N/2, 0)
objp[39] = (4*N+N/2, 3*N+N/2, 0)
objp[40] = (5*N, 0  , 0)
objp[41] = (5*N, N , 0)
objp[42] = (5*N, 2*N, 0)
objp[43] = (5*N, 3*N, 0)
...
images = glob.glob('circle_calibration/*.jpg')
